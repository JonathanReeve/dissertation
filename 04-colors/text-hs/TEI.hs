...
-- | Convert Pandoc document to string in TEI format.
...
elementToTEI opts lvl (Sec _ _num attr title elements) = do
  ...
  let elements' = if null elements
      ...
      divType = if (isChapter attr) then "chapter" else
          case lvl of
                  n | n == -1          -> "part"
                    | n == 0           -> "chapter"
...
-- Test if a div's attribute contains "chapter"
isChapter :: Attr -> Bool
isChapter (id, _, _) = if T.isPrefixOf "chapter" (T.pack id) then True else False
