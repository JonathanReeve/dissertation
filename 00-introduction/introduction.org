...
To simplify computational corpus creation, I decided to use those works which the Library of Congress labels "PR," for British Literature, although I'll be using the term /British literature/ to describe it. This choice is necessary, due to the limitations of Project Gutenberg metadata, which I'll explain.
...
To accomplish that, I created a project called [[http://corpus-db.org/][Corpus-DB]], a database and API for electronic texts from Project Gutenberg and other repositories, which augments the metadata from these reporsitories. I developed this project over the course of several years, with the help of a few students and volunteers. To augment the metadata, I used the title and author of the texts to create SPARQL queries to query the graph knowledge databases such as DBPedia and Wikidata. Both dictionary-based knowledge graphs, these databases maintain statements in the form of triples, e.g., ~<Ulysses> <first published> <1922>~. A SPARQL query could thus ask the database to fill in the question marks for a query such as:
